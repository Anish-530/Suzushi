...
module.exports={
    ...
    description: 'Tired of removing roles manually? Don\'t worry, Suzushi can help you removing role from a user, swiftly.',
    aliases: ['arole'],
    usage: 'su removerole <mention someone> <mention a role>',
    run: async(bot, message, args)=>{
        try{
            ...
            if(!role || !member) {
                ...
                \`\`\`fix
su removerole <mention a user> <mention a role or provide the role\'s name>
OR
su removerole <provide the user\'s ID> <provide a role's ID>\`\`\``)
                return message.channel.send(error);
            }
            else if(role && member){
                if(!member.roles.cache.has(role.id)) {
                    let me = new Discord.MessageEmbed()
                    .setTitle(`<:notgood:776121645709525002> ${member.user.tag}, do not have the role!`)
                    .setColor('#2f3136')
                ...
                .setColor('#2f3136')
                .setDescription(`<:good:776121655528783964> The role ${role} was removed from **${member.user.tag}**`)
