...
module.exports={
    name: 'setgoodbye',
    category: 'guild',
    description: 'set the goodbye channel, guess what? We have a beautiful goodbye Canvas',
    aliases: ['sgc', 'setgc'],
    usage: 'su setgoodbye <mention a channel>',
    run: async(bot, message, args)=>{
        ...
        if(!channel) {
                ...
                return new Promise(async (resolve, reject) => {
                    ...
                    if (collected.size === 0) reject(`Timeout`);
                        ...
                        if (
                            ...
                            let error = await message.channel.send('Please mention a channel, for the goodbye messages. You can type \`cancel\`, if you want to cancel the setup.');
            ...
            questionMessage = await message.channel.send('Which channel do you want to set for goodbye messages? You can type \`cancel\`, if you want to cancel the setup.');
            try {
                let chan = message.mentions.channels.first();
                chan = await verifyChannel();
                await db.set(`goodchannel_${message.guild.id}`, chan.id)
                const wasset1 = new Discord.MessageEmbed()
                .setTitle(`Goodbye Channel`)
                .setDescription(`<:good:776121655528783964> Goodbye channel for **${message.guild.name}**, is now set as ${chan}`)
                .setColor('#2f3136')
                ...
                if (e === `Exiting`) {
                    return message.channel.send(`Successfully cancelled the setting of channel for goodbye messages`).then(message => message.delete({ timeout: 10000 }));
                } else if (e === `Timeout`) {
                    await message.channel.send(`Goodbye messages setup has timed out. Please run the command again.`,);
                }
            }
        }
        else if(channel){
