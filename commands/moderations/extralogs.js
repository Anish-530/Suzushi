...
module.exports={
    ...
    description: 'Want some extra logs? You can switch this on.',
    aliases: ['el'],
    usage: 'su extralogs <mention a channel>',
        ...
        try{
            ...
            if(!channel) {
                ...
                try {
                    ...
                    await db.set(`extralogchannel_${message.guild.id}`, chan.id)
                    const wasset1 = new Discord.MessageEmbed()
                    .setTitle(`Extra logs`)
                    .setDescription(`<:good:776121655528783964> Extra logs for **${message.guild.name}**, is now set as ${chan}`)
                    .setColor('#2f3136')
                ...
                db.set(`extralogchannel_${message.guild.id}`, channel.id)
                const wasset = new Discord.MessageEmbed()
                .setTitle(`Extra logs`)
                .setDescription(`<:good:776121655528783964> Extra logs for **${message.guild.name}**, is now set as ${channel}`)
                .setColor('#2f3136')
                .setTimestamp(new Date())
                .setFooter('Suzushi', bot.user.avatarURL())
