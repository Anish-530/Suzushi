...
module.exports={
    ...
    aliases: ['sb'],
    usage: 'su softban <mention the user you want to ban> <number of days for ban> [reason]',
    run: async(bot, message, args)=>{
        try{
        let chx = db.fetch(`banlogchannel_${message.guild.id}`);
        ...
        if (!userb || !number_of_days) {
            ...
            \`\`\`fix
su ban <mention an user> <number of days for ban> [reason]
OR
su ban <the ID of an user> <number of days for ban> [reason]\`\`\``)
